...
const debug = Debug('AssocAttrsManySpec');
...
describe('AssocAttrsMany', function () {
  ...
  describe('#generate', function () {
    ...
    it('passes arguments to attrsMany correctly', asyncFunction(async function () {
      sinon.spy(factoryGirl, 'attrsMany');
      const dummyAttrs = {};
      const dummyBuildOptions = {};
      const assocAttrsMany = new AssocAttrsMany(factoryGirl, 'model', 10, dummyAttrs, dummyBuildOptions);
      await assocAttrsMany.generate();
      expect(factoryGirl.attrsMany).to.have.been.calledWith('model', 10, dummyAttrs, dummyBuildOptions);
      factoryGirl.attrsMany.restore();
    }));
    ...
    it('resolves to array returned by attrsMany', asyncFunction(async function () {
      const assocAttrsMany = new AssocAttrsMany(factoryGirl, 'model', 10);
    ...
    it('resolves to array of keys if key is set', asyncFunction(async function () {
      const assocAttrsMany = new AssocAttrsMany(factoryGirl, 'model', 10, 'name');
      const models = await assocAttrsMany.generate();
      expect(models).to.have.lengthOf(2);
